package com.grietenenknapen.sithandroid.ui.views.starview;
...
public class ParticlesGenerator implements Runnable {
    ...
    public void run() {
        final FloatBuffer vertexBuffer = makeInterleavedBuffer(
                ...
                generateTextureUv(),
                generateMisc(),
                com.pddstudio.starview.opengl.ParticleSystem.PARTICLE_COUNT
        );
    ...
    private float[] generatePositions() {
        final int size = com.pddstudio.starview.opengl.ParticleSystem.PARTICLE_COUNT * com.pddstudio.starview.opengl.ParticleSystem.POS_DATA_SIZE * 6;
        ...
        int offset = 0;
        for (int p = 0; p < com.pddstudio.starview.opengl.ParticleSystem.PARTICLE_COUNT; p++) {
            ...
            final int elementsPerPoint = com.pddstudio.starview.opengl.ParticleSystem.POS_DATA_SIZE;
    ...
    private float[] generateTextureUv() {
        int size = com.pddstudio.starview.opengl.ParticleSystem.TEXTURE_COORDS_DATA_SIZE * 6;
        ...
        final int elementsPerPoint = com.pddstudio.starview.opengl.ParticleSystem.TEXTURE_COORDS_DATA_SIZE;
    ...
    private float[] generateMisc() {
        final int dataLength = com.pddstudio.starview.opengl.ParticleSystem.PARTICLE_COUNT * com.pddstudio.starview.opengl.ParticleSystem.MISC_DATA_SIZE * 6;

        float[] starData = new float[dataLength];

        int offset = 0;
        for (int i = 0; i < com.pddstudio.starview.opengl.ParticleSystem.PARTICLE_COUNT; i++) {
    ...
    private FloatBuffer makeInterleavedBuffer(float[] posData,
        ...
        for (int i = 0; i < numStars; i++) {
            for (int v = 0; v < 6; v++) {
                interleavedBuffer.put(posData, positionOffset, com.pddstudio.starview.opengl.ParticleSystem.POS_DATA_SIZE);
                positionOffset += com.pddstudio.starview.opengl.ParticleSystem.POS_DATA_SIZE;

                interleavedBuffer.put(uvData, uvOffset, com.pddstudio.starview.opengl.ParticleSystem.TEXTURE_COORDS_DATA_SIZE);
                uvOffset = (uvOffset + com.pddstudio.starview.opengl.ParticleSystem.TEXTURE_COORDS_DATA_SIZE ) % uvData.length;

                interleavedBuffer.put(miscData, starDataOffset, com.pddstudio.starview.opengl.ParticleSystem.MISC_DATA_SIZE);
